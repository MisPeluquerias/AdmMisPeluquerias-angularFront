...
export class EditHomeComponent implements OnInit {
  ...
  processHours(hours_old: string): void {
    ...
    this.dias = days.map((day) => {
      ...
      return {
        nombre,
        mananaInicio:
          manana && manana !== 'Cerrado' ? manana.split(' ')[1] : '',
      ...
      .map((dia) => {
        ...
        if (!dia.cerradoManana) {
          if (dia.mananaInicio && dia.mananaFin) {
            ...
            this.toastr.error(
              `Las horas de la mañana para ${dia.nombre} no son válidas`
            );
            hasError = true;
          }
        }
        ...
        if (!dia.cerradoTarde) {
          if (dia.tardeInicio && dia.tardeFin) {
            ...
            this.toastr.error(
              `Las horas de la tarde para ${dia.nombre} no son válidas`
            );
            hasError = true;
          }
        }
  ...
  updateSalon(): void {
    if (!this.salonData.id_city) {
      this.toastr.warning(
        'Por favor, seleccione una ciudad para actualizar los datos'
      );
      return;
    }
    console.log(this.salonData);
  ...
  uploadImg(): void {
    if (this.images.length >= 3) {
      this.toastr.error(
        '<i class="las la-info-circle"> No se pueden subir mas de tres imagenes.</i>'
      );
      return
    }
    ...
    if (!this.selectedFile) {
      this.toastr.error(
        '<i class="las la-info-circle">Por favor, seleccione una imagen primero.</i>'
      );
      return;
    }
  ...
  deleteImage(imageId: number): void {
    this.editHomeService.deleteImage(imageId).subscribe(
      (response) => {
  ...
  checkboxPrincipalImg(selectedImage: any): void {
    if (selectedImage.file_id != null) {
      // Desmarcar todas las imágenes excepto la seleccionada
      this.images.forEach((image) => {
        if (image.file_id !== selectedImage.file_id) {
          image.file_principal = false;
          this.editHomeService
            .updatePrincipalImage(image.file_id, '0')
            .subscribe();
        }
      });

      // Marcar la imagen seleccionada como principal
      selectedImage.file_principal = true;
      this.editHomeService
        .updatePrincipalImage(selectedImage.file_id, '1')
        .subscribe();
    }
  }
}
